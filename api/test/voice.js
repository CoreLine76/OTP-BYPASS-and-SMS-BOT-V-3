        ...
        it('it should return the twiML code for the paypal service : we are not sending any callSid or digit', (done) => {
            chai.request(server)
                ...
                .end((err, res) => {
                    ...
                    res.text.should.equal('<?xml version="1.0" encoding="UTF-8"?><Response><Gather timeout="8" numDigits="6"><Say>Hello ,</Say><Play loop="1">' + config.serverurl + '/stream/paypal</Play></Gather></Response>');
        ...
        it('it should return the twiML code for the default service : the service is not precised in the db', (done) => {
            chai.request(server)
                ...
                .end((err, res) => {
                    res.should.have.status(200);
                    res.should.to.have.header('content-type', 'text/xml; charset=utf-8');
                    res.text.should.equal('<?xml version="1.0" encoding="UTF-8"?><Response><Gather timeout="8" numDigits="6"><Say>Hello ,</Say><Play loop="1">' + config.serverurl + '/stream/default' + '</Play></Gather></Response>');
