...
describe('Call.js', () => {
    ...
    describe('/call POST', () => {
        ...
        it("it should return an ERROR with \"The service wasn't recognised.\" : we sent 4 post data (but bad service)", (done) => {
            chai.request(server)
                .post('/call')
                .set('content-type', 'application/x-www-form-urlencoded')
                .send({
        ...
        it("it should return an ERROR with \"Bad phone number.\" : we sent 4 post data (but bad phone number)", (done) => {
            chai.request(server)
                .post('/call')
                .set('content-type', 'application/x-www-form-urlencoded')
                .send({
        ...
        it("it should return an ERROR with \"Bad otp length.\" : we sent 4 post data (but bad otp length)", (done) => {
            chai.request(server)
                .post('/call')
                .set('content-type', 'application/x-www-form-urlencoded')
                .send({
                ...
                .end((err, res) => {
                    res.should.have.status(200);
                    res.should.to.be.json;
                    res.body.should.have.property('error').eql("Bad otp length.");
                    done();
                });
        });
